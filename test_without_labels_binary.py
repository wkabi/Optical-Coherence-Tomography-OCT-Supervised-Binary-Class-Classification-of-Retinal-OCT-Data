'''
Project OCT: CNV-NORMAL binary classification (Testing)
...
parser.add_argument('--data_path', type=str, default='/home/wkabir/Data_OCTKermany/images/', help='path data')
parser.add_argument('--csv_val', type=str, default='data/CNV_val_dataset_Waziha_binlabel.csv', help='path to val data csv')
parser.add_argument('--csv_test', type=str, default='data/CNV_Test_dataset_Nov8.csv', help='path to test data csv')
parser.add_argument('--model_name', type=str, default='resnet18', help='selected architecture')
parser.add_argument('--load_path', type=str, default='experiments/model/cycle...', help='path to saved model')
...
if __name__ == '__main__':
    ...
    probs = probs.squeeze()
    vl_auc, vl_acc, vl_f1 = evaluate_bin_cls(labels, preds, probs, print_conf=print_conf,
                                             class_names=class_names, text_file=text_file)


    if n_classes ==1 :
        print('mAUC: {:.2f} - Accuracy: {:.2f} - F1-score: {:.2f}'.format(100 * vl_auc, 100 * vl_acc, 100 * vl_f1))
        im_list = list(val_loader.dataset.im_list)
        im_list = [n.replace(data_path,'') for n in im_list]
        df = pd.DataFrame(zip(im_list, probs, labels),
                          columns=['image_id', 'CNV', 'label'])
    ...
    if n_classes == 1:
        ...
        im_list = [n.replace(data_path,'') for n in im_list]
        df_nogt = pd.DataFrame(zip(im_list, probs), columns=['image_id', 'CNV'])
    else: sys.exit('Wrong number of classes when saving dataframe')
