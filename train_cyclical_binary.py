'''
Project OCT: CNV-NORMAL binary classification (Training & Validation)
...
from utils.get_loaders import get_train_val_bin_cls_loaders, modify_dataset_bin
...
parser.add_argument('--csv_train', type=str, default='data/CNV_train_dataset_Waziha_binlabel.csv', help='path to training data csv')
parser.add_argument('--data_path', type=str, default='/home/wkabir/Data_OCTKermany/images/', help='path to training data csv')
parser.add_argument('--model_name', type=str, default='resnet50', help='architecture')
parser.add_argument('--neg_classes', type=str, default='0/1', help='negative classes')
parser.add_argument('--pos_classes', type=str, default='2/3/4/5', help='positive classes')
    ...
    with trange(len(loader)) as t:
        ...
        for i_batch, (inputs, labels, _) in enumerate(loader):
            ...
            if train:  # only in training mode
                if isinstance(optimizer, torch_optimizer.Adahessian):
                    loss.backward(create_graph=True)  # create_graph=True is necessary for Hessian calculation
                else:
                    loss.backward()
...
def train_one_cycle(train_loader, oversample, model, criterion, optimizer, scheduler, cycle=0):
    ...
    train_loader_MOD = modify_dataset_bin(train_loader, csv_train_path=csv_train_path, keep_samples=oversample, see_classes=True)
    ...
    for epoch in range(cycle_len):
        ...
        train_loader_MOD = modify_dataset_bin(train_loader, csv_train_path=csv_train_path, keep_samples=oversample, see_classes=False)
...
def train_model(model, optimizer, train_criterion, val_criterion, do_mixup, train_loader, val_loader,
    ...
    best_loss, best_auc, best_acc, best_f1, best_cycle, best_models = 10, 0, 0, 0, 0, []
    ...
    for cycle in range(n_cycles):
        ...
        tr_auc, tr_acc, tr_f1 = evaluate_bin_cls(tr_labels, tr_preds, tr_probs, print_conf=print_conf,
                                                              class_names=class_names, text_file=text_file_train, loss=tr_loss)
        vl_auc, vl_acc, vl_f1 = evaluate_bin_cls(vl_labels, vl_preds, vl_probs, print_conf=print_conf,
                                                              class_names=class_names, text_file=text_file_val, loss=vl_loss)

        print('Train||Val Loss: {:.4f}||{:.4f} - mAUC: {:.2f}||{:.2f} - ACC: {:.2f}||{:.2f} - F1: {:.2f}||{:.2f}'.format(
                                    tr_loss, vl_loss, 100*tr_auc, 100*vl_auc, 100*tr_acc, 100*vl_acc, 100*tr_f1, 100*vl_f1))

        # check if performance was better than anyone before and checkpoint if so
        if metric == 'loss':  monitoring_metric = vl_loss
        elif metric == 'acc': monitoring_metric = vl_acc
        ...
        all_tr_f1s.append(tr_f1)
        all_vl_f1s.append(vl_f1)
        ...
        if is_better(monitoring_metric, best_monitoring_metric):
            ...
            best_loss, best_acc, best_f1, best_auc, best_cycle = vl_loss, vl_acc, vl_f1, vl_auc, cycle+1
        ...
        import shutil
        if exp_path is not None:
            s_name = 'cycle_{}_mAUC_{:.2f}_MCC_{:.2f}_F1_{:.2f}'.format(str(cycle + 1).zfill(2),100*vl_auc, 100*vl_acc, 100*vl_f1)
    ...
    return best_auc, best_acc, best_f1, all_tr_aucs, all_vl_aucs, all_tr_accs, all_vl_accs, \
...
if __name__ == '__main__':
    ...
    if len(oversample) != 2: sys.exit('oversample must be a number or a tuple of len {:d}'.format(n_classes+1))

    neg_classes, pos_classes = args.neg_classes, args.pos_classes
    if len(neg_classes) == 1: neg_classes = [int(neg_classes)]
    else:
        neg_classes = neg_classes.split('/')
        neg_classes = list(map(int, neg_classes))
    if len(pos_classes) == 1: pos_classes = [int(pos_classes)]
    else:
        pos_classes = pos_classes.split('/')
        pos_classes = list(map(int, pos_classes))
    ...
    train_loader, val_loader = get_train_val_bin_cls_loaders(csv_path_train=csv_train, csv_path_val=csv_val,
                                                             neg_classes=neg_classes, pos_classes=pos_classes,
    ...
    if optimizer_choice == 'adam':
        ...
        optimizer = torch.optim.SGD(model.parameters(), lr=max_lr, weight_decay=args.wd, momentum=args.mom)
    elif optimizer_choice == 'sgd_sam':
        base_optimizer = torch.optim.SGD  # define an optimizer for the "sharpness-aware" update
        optimizer = SAM(model.parameters(), base_optimizer, lr=max_lr, momentum=args.mom, weight_decay=args.wd)
    else:
        sys.exit('please choose a valid optimizer')

    print('* Instantiating {} optimizer'.format(type(optimizer).__name__))

    if pretrained_weights is True:
        weights_path = osp.join('experiments/cyclical/dme_bin_0_vs12/bit50_sgd_no_dme_vs_dme_11_F1/cycle_05_mAUC_95.56_MCC_91.54_F1_77.93/')
        try:
        ...
        #     for i, param_group in enumerate(optimizer.base_optimizer.param_groups):
        #         param_group['lr'] = max_lr
        #     print('* Pretrained optimizer loaded')
        # except:
        #     print('Checkpointed optimizer not compatible with requested optimizer, discarding checkpoint')
    if isinstance(optimizer, SAM):
    ...
    train_criterion, val_criterion = torch.nn.BCEWithLogitsLoss(), torch.nn.BCEWithLogitsLoss()
    do_mixup = args.do_mixup

    print('* Instantiating loss function', str(train_criterion))
    print('* Starting to train\n','-' * 10)
    start = time.time()
    b_mauc, b_acc, b_f1, tr_aucs, vl_aucs, \
    tr_accs, vl_accs, tr_f1s, vl_f1s, tr_ls, vl_ls, b_cycle=train_model(model, optimizer, train_criterion, val_criterion, do_mixup,
    ...
    if do_not_save is False:
        with open(osp.join(experiment_path, 'val_metrics.txt'), 'w') as f:
            print('Best mAUC = {:.2f}\nBest ACC = {:.2f}\nBest F1 = {:.2f}\nBest cycle = {}\n'.format(
                100 * b_mauc, 100 * b_acc, 100 * b_f1, b_cycle), file=f)
            for j in range(len(vl_aucs)):
                # print('Epoch = {} -> mAUC={:.2f}/{:.2f}, '.format(j+1, 100*tr_aucs[j], 100*vl_aucs[j]),file=f)
                print('Cycle = {} -> mAUC={:.2f}/{:.2f}, ACC: {:.2f}/{:.2f}, F1: {:.2f}/{:.2f}, Loss: {:.4f}/{:.4f}'.format(j+1,
                                                                                                         100*tr_aucs[j], 100*vl_aucs[j],
                                                                                                         100*tr_accs[j], 100*vl_accs[j],
                                                                                                         100*tr_f1s[j], 100*vl_f1s[j],
                                                                                                         tr_ls[j], vl_ls[j]),file=f)
